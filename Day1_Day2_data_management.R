...
#dim(url)             "&separator=comma&csv-lang=en&startPeriod=2012")
...
table(is.na(Animal_Data$heart_rate_perminute))
table(is.na(Animal_Data$rectal_temp))

#outlier detection using box plots
boxplot(Animal_Data$age)
boxplot(Animal_Data$heart_rate_perminute)
boxplot(Animal_Data$rectal_temp)

#data distributions using visual approach and statistical tests
hist(Animal_Data$age)
hist(Animal_Data$dam_milkyield_liters)
hist(Animal_Data$rectal_temp)
...
shapiro.test(Animal_Data$heart_rate_perminute)
str(Animal_Data)
shapiro.test(Animal_Data$rectal_temp)
